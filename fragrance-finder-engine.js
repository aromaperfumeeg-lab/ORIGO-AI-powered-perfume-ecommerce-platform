(function(global){
  ...
  const scoreMap=(value,ids)=>Object.fromEntries(ids.map(id=>{const raw=value?.[id];return[id,raw==null?null:Math.max(0,Math.min(100,Number(raw)))]}));
  ...
  function buildFinderProfileFromProduct(product={}){
    const bundle=bundleData(product);
    ...
    const seasons=scoreMap(product.seasonScores,["winter","autumn","spring","summer"]);
    const usageTime=scoreMap(product.usageTimeScores,["day","night"]);
    const longevity=product.performance?.longevityHours??product.performance?.longevity??product.performanceInsights?.editorialDetails?.longevityHours??bundle.longevityHours??null;
    const familyAr=product.fragranceFamilyAr||product.familyAr||"",familyEn=product.fragranceFamilyEn||product.familyEn||"";
    const families=product.families?.length?array(product.families).map(pair):[{ar:familyAr,en:familyEn}].filter(item=>item.ar||item.en);
    const characterAr=array(product.scentCharacterAr),characterEn=array(product.scentCharacterEn),characterLength=Math.max(characterAr.length,characterEn.length);
    const scentCharacter=Array.from({length:characterLength},(_,index)=>({ar:String(characterAr[index]||""),en:String(characterEn[index]||"")})).filter(item=>item.ar||item.en);
    const occasionLabels=array(product.occasionLabels).map(pair).filter(item=>item.ar||item.en),occasions=occasionLabels.length?occasionLabels:array(product.occasions).map(pair);
    return{id:String(product.id||""),product,nameAr:product.nameAr||"",nameEn:product.nameEn||"",brand:product.brand||product.brandEn||product.brandAr||"",gender:canonical(product.gender||bundle.gender),accords,families,scentCharacter,rating:product.rating??product.reviewSummary?.average??null,longevity:longevity==null?null:Number(longevity),sillage:projection(product.performance?.projection??product.performance?.sillage),seasons,usageTime,occasions,concentration:product.concentration||"",image:firstImage(product),price:product.price??null,available:product.inventory?.available>0||product.inventory?.quantity>0,status:product.status||"published",category:product.category||"perfume"};
  }
  ...
  const weights={accords:34,families:16,seasons:12,times:10,occasions:9,scentCharacter:7,sillage:5,longevity:5,gender:2};
  ...
  function scoreFinderProfile(profile,preferences={}){let earned=0,availableWeight=0;const reasons=[];const add=(name,value,reason)=>{if(value==null||!Number.isFinite(value))return;availableWeight+=weights[name];earned+=Math.max(0,Math.min(1,value))*weights[name];if(reason&&value>0)reasons.push(reason)};
    ...
    const season=array(preferences.seasons)[0];if(season&&profile.seasons[season]!=null)add("seasons",profile.seasons[season]/100,{type:"season",value:season,score:profile.seasons[season]});
    ...
    const character=listMatch(array(preferences.scentCharacter),profile.scentCharacter);if(character)add("scentCharacter",character.ratio,character.hit.length?{type:"scentCharacter",values:character.hit}:null);
    const time=array(preferences.times)[0];if(time&&profile.usageTime[time]!=null)add("times",profile.usageTime[time]/100,{type:"time",value:time,score:profile.usageTime[time]});
  ...
  const cache=new Map();
  function signature(product){return JSON.stringify([product.updatedAt,product.status,product.category,product.gender,product.accordProfile,product.families,product.fragranceFamilyAr,product.fragranceFamilyEn,product.scentCharacterAr,product.scentCharacterEn,product.seasonScores,product.usageTimeScores,product.occasions,product.occasionLabels,product.performance,product.images?.[0],product.inventory?.quantity])}
