(() => {
  ...
  function itemRows(editable = false) {
    return `<div class="checkout-items">${model.quote.items.map(item=>`<article class="checkout-line">
      <img src="${esc(item.image || "assets/product-placeholder.svg")}" alt=""/><div class="checkout-line-copy"><b>${esc(ar()?item.nameAr:item.nameEn||item.nameAr)}</b><small>${esc(item.concentration)}${item.size?` · ${esc(item.size)}`:""}</small><span class="stock-ok">${t("متوفر","In stock")}</span></div>
  ...
  function orderItems(order){return `<div class="tracked-items">${order.items.map(item=>`<article><img src="${esc(item.image||"assets/product-placeholder.svg")}" alt=""/><span><b>${esc(item.productName)}</b><small>${esc(item.concentration)}${item.size?` · ${esc(item.size)}`:""}<br/>${item.quantity} × ${money(item.unitPrice)}</small></span><strong>${money(item.lineTotal)}</strong></article>`).join("")}</div>`;}
