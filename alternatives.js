(() => {
  ...
  function savings(item) { const referencePrice=Number(item.reference.referencePrice);const productPrice=Number(item.product.price);return referencePrice>0&&Number.isFinite(productPrice)?Math.max(0,referencePrice-productPrice):null; }
  function moneyOrDash(value) { const amount=Number(value);return Number.isFinite(amount)&&amount>0?money(amount):"—"; }
  function performanceValue(value) { const score=Number(value);return Number.isFinite(score)&&score>0?Math.min(10,Math.max(0,score)):null; }
  ...
  function referenceAvailable(item) { const product=item.referenceProduct;return Boolean(product&&product.status==="published"&&Number(product.inventory?.available??product.inventory?.quantity??1)>0); }
  function referenceActions(item) {
    const product=item.referenceProduct;
    if (!product) return `<em>${t("referenceNotice")}</em>`;
    return `<div class="alt-reference-commerce"><strong>${moneyOrDash(product.price)}</strong><span class="${referenceAvailable(item)?"stock":"out"}">● ${referenceAvailable(item)?t("available"):t("unavailable")}</span><div><button data-alt="reference-product" data-product="${esc(product.id)}">${t("product")} ◇</button><button data-alt="reference-add" data-product="${esc(product.id)}"${referenceAvailable(item)?"":" disabled"}>🛒 ${t("add")}</button></div></div>`;
  }
  ...
  function listingCard(item) {
    return `<article class="alternative-card">
      ...
      <div class="alternative-card-body"><div class="alt-shared-notes">${notes(item).map((note) => `<span>✦ ${esc(note)}</span>`).join("")}</div><p>${esc(label(item, "reason"))}</p><div class="alt-card-commerce">${stars(item)}<div><strong>${money(item.product.price)}</strong>${savings(item)==null?"":`<small>${t("savings")} ${money(savings(item))}</small>`}</div></div>${actionButtons(item)}</div>
    </article>`;
  }
  ...
  function renderListing() {
    ...
    root.innerHTML = `<nav class="alt-breadcrumb"><a href="/" data-alt="home">${lang() === "ar" ? "الرئيسية" : "Home"}</a><span>‹</span><b>${t("alternatives")}</b></nav>
      ...
      <section class="alternatives-search smart-finder-search"><header><div><label for="alternatives-search">${lang() === "ar" ? "ابحث عن أي عطر لتجد أفضل البدائل الذكية" : "Search any fragrance to find its smartest alternatives"}</label><p>${lang() === "ar" ? "اكتب اسم العطر الذي تحبه وسنقترح لك أفضل البدائل المشابهة بسعر مناسب وأداء أفضل." : "Enter a fragrance you love and we will rank the closest available alternatives."}</p></div><aside><i>⌕</i><b>${lang() === "ar" ? "مكتشف البديل الذكي" : "Smart alternative finder"}</b><small>${lang() === "ar" ? "نقارن الرائحة والأداء والسعر" : "Scent, performance and value"}</small></aside></header><div><span>⌕</span><input id="alternatives-search" type="search" value="${esc(model.query)}" placeholder="${t("searchHint")}" autocomplete="off"/><button data-alt="search">${lang() === "ar" ? "بحث" : "Search"}</button></div><div class="alternatives-suggestions" id="alternatives-suggestions" hidden></div></section>
      <section class="alternative-search-examples"><header><span></span><h2>${lang() === "ar" ? "أمثلة على عطور يبحث عنها عملاؤنا" : "Popular searches from our customers"}</h2><span></span></header><div>${model.payload.items.slice(0,5).map(item=>`<button data-alt="suggestion" data-value="${esc(label(item.reference,"name"))}"><img src="${esc(item.reference.image)}" alt=""/><b>${esc(label(item.reference,"name"))}</b><small>${esc(item.reference.brand)}</small><i>⌕</i></button>`).join("")}</div></section>
  ...
  function renderMultiCompare(items) {
    root.innerHTML = `<nav class="alt-breadcrumb"><a href="/alternatives" data-alt="open">${t("returnAlternatives")}</a></nav><header class="comparison-title"><small>${t("eyebrow")}</small><h1>${t("compareSelected")}</h1></header><section class="multi-alternative-compare">${items.map((item)=>`<article><img src="${esc(item.product.image)}" alt="${esc(label(item.product,"name"))}"/><h2>${esc(label(item.product,"name"))}</h2><small>${esc(item.product.brand)}</small>${similarityRing(item,true)}<dl><div><dt>${t("reference")}</dt><dd>${esc(label(item.reference,"name"))}</dd></div><div><dt>${t("family")}</dt><dd>${esc(label(item.product,"family")||label(item.reference,"family"))}</dd></div><div><dt>${t("performance")}</dt><dd>${Number(item.scoreBreakdown?.performance)>0?`${number(item.scoreBreakdown.performance)}%`:"—"}</dd></div><div><dt>${t("price")}</dt><dd>${moneyOrDash(item.product.price)}</dd></div><div><dt>${t("savings")}</dt><dd>${savings(item)==null?"—":money(savings(item))}</dd></div></dl>${actionButtons(item,true)}</article>`).join("")}</section><p class="alternatives-disclaimer">ⓘ ${t("disclaimer")}</p>`;
  }
  ...
  function scoreBar(labelText, referenceValue, alternativeValue) {
    const referenceScore=performanceValue(referenceValue),alternativeScore=performanceValue(alternativeValue);
    const metric=(side,value)=>`<div><span>${side}</span><i><em style="width:${value==null?0:value*10}%"></em></i><strong>${value==null?"—":`${number(value,{maximumFractionDigits:1})}/10`}</strong></div>`;
    return `<div class="comparison-score"><b>${labelText}</b>${metric(t("reference"),referenceScore)}${metric(t("availableAlternative"),alternativeScore)}</div>`;
  }
  ...
  function comparisonRows(item) {
    ...
    const rows = [
      ...
      [t("price"), moneyOrDash(ref.referencePrice), moneyOrDash(product.price)], [t("savings"), "—", savings(item)==null?"—":money(savings(item))], [t("similarity"), "—", `${item.similarity}%`]
    ];
  ...
  function renderCompare(item) {
    ...
    root.innerHTML = `<nav class="alt-breadcrumb"><a href="/" data-alt="home">${lang() === "ar" ? "الرئيسية" : "Home"}</a><span>‹</span><a href="/alternatives" data-alt="open">${t("alternatives")}</a><span>‹</span><b>${esc(label(ref, "name"))}</b></nav>
      ...
      <section class="comparison-hero"><article class="comparison-product reference${item.referenceProduct?" is-purchasable":""}"><span>${item.referenceProduct?t("available"):t("reference")}</span><img src="${esc(item.referenceProduct?.image||ref.image)}" alt="${esc(label(ref,"name"))}"/><div><h2>${esc(label(ref,"name"))}</h2><b>${esc(ref.brand)}</b><p>${esc(item.referenceProduct?.concentration||ref.concentration)} · ${esc(item.referenceProduct?.sizes?.[0]||ref.size)}</p>${referenceActions(item)}</div></article><div class="comparison-center">${similarityRing(item)}<b>${t("highMatch")}</b><small>${t("confidence")}: ${item.confidence}%</small></div><article class="comparison-product alternative"><span>${t("suggested")}</span><img src="${esc(product.image)}" alt="${esc(label(product,"name"))}"/><div><h2>${esc(label(product,"name"))}</h2><b>${esc(product.brand)}</b><p>${esc(product.concentration)} · ${esc(product.sizes?.[0] || "")}</p>${stars(item)}<strong>${money(product.price)}</strong><em class="stock">● ${available(item) ? t("available") : t("unavailable")}</em>${actionButtons(item)}</div></article></section>
      <section class="comparison-insights"><article><span>✦</span><div><h2>${t("reason")}</h2><p>${esc(label(item,"reason")||"—")}</p><small>${esc(relationLabel(item))}</small></div></article><article class="saving"><span>♕</span><div><h2>${t("savings")}</h2><strong>${savings(item)==null?"—":money(savings(item))}</strong><p>${t("originalPrice")}: ${moneyOrDash(ref.referencePrice)}</p></div></article><article><span>◷</span><div><h2>${t("performance")}</h2>${scoreBar(t("longevity"), ref.performance?.longevity, comp.longevity)}${scoreBar(t("projection"), ref.performance?.projection, comp.projection)}</div></article></section>
      ...
      <section class="inspired-section"><header><span></span><h2>${t("inspiredTitle")}</h2><span></span></header><div class="inspired-pair"><article><b>${t("referenceNotice")}</b><img src="${esc(ref.image)}" alt=""/><h3>${esc(label(ref,"name"))}</h3></article><div>⇄</div><article class="available"><b>${t("availableAlternative")}</b><img src="${esc(product.image)}" alt=""/><h3>${esc(label(product,"name"))}</h3><strong>${item.similarity}%</strong>${actionButtons(item,true)}</article></div><div class="inspired-facts"><article><h3>${t("commonNotes")}</h3><div>${notes(item).length?notes(item).map(n=>`<span>✦ ${esc(n)}</span>`).join(""):"—"}</div></article><article><h3>${t("performance")}</h3><p>${t("longevity")}: ${performanceValue(comp.longevity)==null?"—":`${number(performanceValue(comp.longevity),{maximumFractionDigits:1})}/10`}</p><p>${t("projection")}: ${performanceValue(comp.projection)==null?"—":`${number(performanceValue(comp.projection),{maximumFractionDigits:1})}/10`}</p></article><article><h3>${t("priceDifference")}</h3><p>${t("originalPrice")}: ${moneyOrDash(ref.referencePrice)}</p><b>${t("alternativePrice")}: ${moneyOrDash(product.price)}</b></article><article><h3>${t("suitableSeason")}</h3><p>${ref.seasons?.length?ref.seasons.map(s=>`<span>${s === "summer" ? "☀" : s === "winter" ? "❄" : "❧"} ${esc(s)}</span>`).join(" "):"—"}</p></article></div></section>
  ...
  function suggestions(value) {
    ...
    const popular = (model.payload?.items || []).slice(0,5);
    host.innerHTML = items.length ? `<div class="alt-suggestion-columns"><section><small>${lang()==="ar"?"أشهر عمليات البحث":"Popular searches"}</small>${popular.map(item=>`<button data-alt="suggestion" data-value="${esc(label(item.reference,"name"))}"><i>⌕</i><span><b>${esc(label(item.reference,"name"))}</b><small>${esc(item.reference.brand)}</small></span></button>`).join("")}</section><section><small>${t("similarSearch")}</small>${items.map(item=>`<button data-alt="suggestion" data-value="${esc(label(item.reference,"name"))}"><img src="${esc(item.reference.image)}" alt=""/><span><b>${esc(label(item.reference,"name"))}</b><small>${esc(item.reference.brand)}</small></span><em>${item.similarity}%</em></button>`).join("")}</section></div><footer><button data-alt="search">${lang()==="ar"?`عرض جميع النتائج للبحث عن “${esc(value)}”`:`View all results for “${esc(value)}”`} ←</button></footer>` : `<p>${t("noResults")}</p>`;
    host.hidden = false;
  }
  ...
  document.addEventListener("click", (event) => {
    ...
    if (action === "reference-product") { const item=model.payload.items.find(i=>i.referenceProduct?.id===el.dataset.product); store().showProductDetails(store().getProduct(el.dataset.product)||item?.referenceProduct); postEvent(item,"product_view"); }
    if (action === "reference-add") { const item=model.payload.items.find(i=>i.referenceProduct?.id===el.dataset.product); const product=store().getProduct(el.dataset.product)||item?.referenceProduct;if(product&&referenceAvailable(item))store().addToCart(product);postEvent(item,"add_to_cart"); }
