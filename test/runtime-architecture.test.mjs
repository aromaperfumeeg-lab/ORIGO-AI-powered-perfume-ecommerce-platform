...
test("home loads production storefront core without admin editor or finder runtimes", async () => {
  const [html, loader, core, serviceWorker] = await Promise.all([read("../index.html"), read("../runtime-loader.js"), read("../chunks/storefront-core.min.js"), read("../sw.js")]);
...
test("notes library keeps search intentional and hides internal artwork diagnostics", async () => {
  ...
  assert.match(render, /عدد النوتات العطرية/);
  assert.match(render, /عدد العائلات العطرية/);
...
test("initial asset budgets keep admin and finder out of storefront", async () => {
  ...
  assert.ok(await bytes(initialScripts) < 300 * 1024, "initial JS raw budget");
  assert.ok(await bytes(initialStyles) < 300 * 1024, "initial CSS raw budget");
