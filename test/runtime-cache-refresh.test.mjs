...
test("runtime checks the service worker immediately and reloads once after an update", () => {
  ...
  assert.match(deferred, /origoRuntimeReload-v156/);
  assert.match(deferred, /location\.reload\(\)/);
  assert.match(html, /deferred-modules\.js\?v=19/);
  assert.match(worker, /origo-static-v156/);
  assert.match(worker, /deferred-modules\.js\?v=19/);
});
