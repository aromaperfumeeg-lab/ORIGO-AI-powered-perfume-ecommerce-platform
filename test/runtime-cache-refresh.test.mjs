...
test("runtime checks the service worker immediately and reloads once after an update", () => {
  ...
  assert.match(deferred, /origoRuntimeReload-v153/);
  assert.match(deferred, /location\.reload\(\)/);
  assert.match(html, /deferred-modules\.js\?v=16/);
  assert.match(worker, /origo-static-v153/);
  assert.match(worker, /deferred-modules\.js\?v=16/);
});
