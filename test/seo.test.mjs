...
test("Merchant offer markup references truthful global shipping and return policies",()=>{const data=buildProductStructuredData(product);assert.equal(data.offers.priceCurrency,"EGP");assert.equal(data.offers.hasMerchantReturnPolicy["@id"],"https://origoscents.com/policies/returns#policy");assert.equal(data.offers.shippingDetails.hasShippingService["@id"],"https://origoscents.com/policies/shipping#service");const home=buildHomepageStructuredData();const org=home.find(item=>item["@type"]==="Organization");assert.equal(org.hasMerchantReturnPolicy.merchantReturnDays,7);assert.equal(org.hasMerchantReturnPolicy.applicableCountry,"EG");const condition=org.hasShippingService.shippingConditions[0];assert.ok(condition.transitTime);assert.equal(condition.deliveryTime,undefined);assert.equal(condition.shippingRate,undefined);assert.equal(condition.transitTime.duration.maxValue,4)});
test("policy routes have indexable metadata and visible policy content",()=>{const shell='<html><head><title>x</title><meta name="description" content="x"></head><body><div class="origo-home" id="home"></div><main id="storefront-main"></main></body></html>';for(const path of ["/policies/shipping","/policies/returns","/policies/privacy","/policies/terms","/about","/contact"]){const seo=seoForRoute(path,[product]);assert.equal(seo.robots,"index,follow");const rendered=injectRouteContent(shell,path,[product]);assert.match(rendered,/origo-policy-page/);assert.match(rendered,/<h1>/);assert.match(rendered,/ORIGO|الخصوصية|الشحن|الإرجاع|الشروط|تواصل/);if(path.endsWith("shipping"))assert.ok(seo.jsonLd.some(item=>item["@type"]==="ShippingService"));if(path.endsWith("returns"))assert.ok(seo.jsonLd.some(item=>item["@type"]==="MerchantReturnPolicy"));}});
...
test("sitemap includes live published URLs, excludes drafts, and is served dynamically",async()=>{
  const xml=buildSitemap([product,{...product,id:"draft",slug:"draft",status:"draft"}]);
  assert.match(xml,/perfume\/asad-bourbon/);
  assert.doesNotMatch(xml,/perfume\/draft|admin|account|checkout/);
  assert.match(xml,/perfumes\/winter/);
  const [server,deploy]=await Promise.all([
    readFile(new URL("../server.mjs",import.meta.url),"utf8"),
    readFile(new URL("../scripts/build-deploy.mjs",import.meta.url),"utf8")
  ]);
  const sitemapRoute=server.indexOf('if (url.pathname === "/sitemap.xml")');
  const staticFallback=server.indexOf("await serveStatic(request, response, url)");
  assert.ok(sitemapRoute>=0&&staticFallback>sitemapRoute,"the dynamic sitemap route must run before static-file serving");
  const route=server.slice(sitemapRoute,staticFallback);
  assert.match(route,/buildSitemap\(listProducts\(\), listProductOptions\("brand"\)\)/);
  const publicFiles=deploy.match(/const publicFiles = \[[\s\S]*?\];/)?.[0]||"";
