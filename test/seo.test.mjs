...
test("policy routes have indexable metadata and visible policy content",()=>{const shell='<html><head><title>x</title><meta name="description" content="x"></head><body><div class="origo-home" id="home"></div><main id="storefront-main"></main></body></html>';for(const path of ["/policies/shipping","/policies/returns","/policies/privacy","/about","/contact"]){const seo=seoForRoute(path,[product]);assert.equal(seo.robots,"index,follow");const rendered=injectRouteContent(shell,path,[product]);assert.match(rendered,/origo-policy-page/);assert.match(rendered,/<h1>/);assert.match(rendered,/ORIGO|الخصوصية|الشحن|الإرجاع|تواصل/);if(path.endsWith("shipping"))assert.ok(seo.jsonLd.some(item=>item["@type"]==="ShippingService"));if(path.endsWith("returns"))assert.ok(seo.jsonLd.some(item=>item["@type"]==="MerchantReturnPolicy"));}});
